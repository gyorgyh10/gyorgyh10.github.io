function osszeadas(e, m) {
    ...
    let elsoSzam = (Math.floor(Math.random() * 100)+ 1)*10 ;
    elso.innerHTML = elsoSzam;
    let masodik = document.querySelector("." + m);
    let masodikSzam = (Math.floor(Math.random() * 100) + 1)*10;
...
audio = new Audio("pics/Marshmello.mp3" );
function megoldas() {
    ...
    if (eredmeny==5) {
        document.querySelector(".kep").src="pics/Gmeno.jpg";
        audio.play();
    }
    if (eredmeny==4) {
        document.querySelector(".kep").src="pics/G4.gif"
    }
    if (eredmeny==3) {
        document.querySelector(".kep").src="pics/G4.gif"
    }
